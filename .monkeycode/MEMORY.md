...
- Instructions:
  - **买入5层过滤网**（量学信号只是第1层，其余为强制过滤项，缺任意一层放弃）：①量学信号层（买入三问+位置<70%+右确认三条件）②大盘环境层（沿用三线+四把体温计+环境三档，不引入海龟200日线以免与"弱市上限2-4成"冲突）③基本面层（非ST/*ST、PE<50、ROE>8%，数据来自权威平台，不得编造）④情绪面层（当日上涨家数>全A约30%，2026年8月≈1590家；连板≥7板=过热禁追、≤3板=冰点谨慎——此阈值无出处且未回测，使用按参考口径标注"待验证"）⑤风险收益比层（≥2:1才出手：风险=入场价−止损价，收益=目标价−入场价）
  ...
  - **止损量化纪律**：单笔最大亏损≤总资金5%；止损后至少2个交易日冷静，不立即反手；止损线=max(关键位−2%, 入场价−2×20日波动率)（波动率止损为可选增强）
  - **失效战法处理**：某战法连续实战10次出现≥5次亏损→立即复盘确认是环境不适还是战法失效；判断失效即暂停教学/下架并给替代方案，禁止硬扛
