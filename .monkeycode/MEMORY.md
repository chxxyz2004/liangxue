...
- Instructions:
  - 时间段与报告类型对应关系（新版，以资金识别为主线）：
    * 15:00至次日9:25（盘前）→ 生成盘前预案（资金流向预判）
    * 9:25-9:30（集合竞价）→ 盘中盯盘启动
    * 9:30-11:30, 13:00-15:00 → "盯盘"=生成盘中资金流向（可手动触发）
    * 11:30-13:00 → "午盘"=生成午间资金识别报告（判断主力/量化意图）
    * 15:00后 → "收盘"=生成收盘资金识别+全面复盘（含次日预案）
  - 报告命名规范：
    * 盘前预案-YYYY-MM-DD.html
    * 盘中盯盘-YYYY-MM-DD.html（盘中触发时生成）
    * 午间资金识别-YYYY-MM-DD.html（替代原午间研判）
    * 收盘资金识别-YYYY-MM-DD.html（替代原收盘复盘，含全面复盘+次日预案）
  - 每日21:00自动生成联网复盘报告：搜索政策面/基本面（官方权威源），结合系统数据生成全面复盘+次日预案
  - 报告命名规范（统一，消除歧义）：
  ...
  - 快速操作行标签：今日盘前预案/今日盘中盯盘/午间研判/收盘资金识别+复盘
